...
class RepoConfig(BaseModel):
    ...
    max_file_size: int = Field(default=1024*1024, description="Max file size to read (1MB)")
    excluded_patterns: List[str] = Field(
        default=[
            "*.pyc", "__pycache__", "*.log", ".git", "node_modules",
            "*.tmp", "*.swp", "*.DS_Store", ".env", "*.key", "*.pem"
        ],
...
class RepoConnector:
    ...
    def __init__(self):
        ...
        self.config = RepoConfig(
            root_path=os.environ["REPO_ROOT"]
        )
        ...
        try:
            ...
            logger.info(f"Repository connector initialized for: {self.config.root_path}")
        except git.InvalidGitRepositoryError:
            logger.warning("Not a valid Git repository, continuing without Git features")
            self.git_repo = None
    ...
    async def handle_repo_read_file(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            return [TextContent(
                type="text",
                text=f"Repository File Content:\n{result_data}"
            )]
            ...
            return [TextContent(
                type="text",
                text=f"Error reading repository file: {str(e)}"
            )]
    
    async def handle_repo_search_files(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            return [TextContent(
                type="text",
                text=f"Repository File Search Results:\n{result_data}"
            )]
            ...
            return [TextContent(
                type="text",
                text=f"Error searching repository files: {str(e)}"
            )]
    
    async def handle_repo_search_content(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
                        ...
                        if content and not content.startswith("["):  # Skip error messages
                            ...
                            if search_query in search_content:
                                ...
                                matches.append({
                                    "path": str(relative_path),
                                    "size": file_path.stat().st_size,
                                    "matching_lines": matching_lines[:10]  # Limit lines per file
                                })
            ...
            return [TextContent(
                type="text",
                text=f"Error searching repository content: {str(e)}"
            )]
    
    async def handle_repo_list_directory(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            return [TextContent(
                type="text",
                text=f"Repository Directory Listing:\n{result_data}"
            )]
            ...
            return [TextContent(
                type="text",
                text=f"Error listing repository directory: {str(e)}"
            )]
    
    async def handle_repo_git_info(self, arguments: Dict[str, Any]) -> List[TextContent]:
        """Handle Git information requests."""
        try:
            if not self.git_repo:
                return [TextContent(
                    type="text",
                    text="Error: Git repository not available"
                )]
            ...
            if info_type == "branch":
                ...
                return [TextContent(
                    type="text",
                    text=f"Error: Unknown info_type: {info_type}"
                )]
            ...
            return [TextContent(
                type="text",
                text=f"Error getting repository Git info: {str(e)}"
            )]