...
def render_chat_interface(assistant: HolographicAssistant) -> None:
    ...
    if prompt := st.chat_input("Enter command or ask a question..."):
        ...
        with st.spinner("🤖 ARIA is thinking..."):
            import asyncio
            try:
                response = asyncio.run(assistant.chat(prompt, st.session_state.chat_history))
            except RuntimeError:
                ...
                if loop.is_running():
                    ...
                    def run_async():
                        ...
                        try:
                            return new_loop.run_until_complete(assistant.chat(prompt, st.session_state.chat_history))
                        finally:
                            new_loop.close()
                    ...
                    response = loop.run_until_complete(assistant.chat(prompt, st.session_state.chat_history))
